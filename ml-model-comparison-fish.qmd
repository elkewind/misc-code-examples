...
My first model will be a logistic regression model with Lasso regularization. 
...
# Bootstrap sampling
#fish_l_boot <- bootstraps(fish_train)
...
# Plot mean rmse and rsq at each penalty
lasso_grid %>%
  collect_metrics() %>%
...
# Finalize the workflow with the specifications and best model for accuracy
# final_lasso <-   wf_l %>% 
#   finalize_workflow(add_model(tune_l_spec),
#                     best_lasso)
...
fit_knn_cv <- wf_knn_tune %>% 
  tune_grid( 
    fish_cv, # does tuning based on folds
...
# fish_knn_pred <- predict(final_knn_fit, new_data = fish_test)
# # Bind to track dataframe
# fish_knn_final <- cbind(fish_test, fish_knn_pred)
...
print(paste0("We see here that our k-nearest neighbors model had a slightly higher accuracy at predicting threat status than the dummy classifier. The accuracy of the model was ", round(final_knn_accuracy, 3), " and the dummy classifier accuracy was ", round(dummy, 3), ". The auc was ", round(final_knn_auc, 3), "."))
```
