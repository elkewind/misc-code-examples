...
We see here that our k-nearest neighbors model had a slightly higher accuracy at predicting threat status than the dummy classifier, but not by much. The accuracy of the model was `r final_knn_accuracy`. This model had an AUC of `r final_knn_auc` which is better than the lasso model. 
...
# Use autoplot() to examine how different parameter configurations relate to accuracy 
autoplot(boost_tree_rs) + theme_light()
...
# Use autoplot() to examine how different parameter configurations relate to accuracy 
autoplot(boost_stoc_rs) + theme_light()
...
ggplot(auc_df, aes(x = models[-1], y = auc)) +
  geom_col(fill = "#69b3a2") +
  theme_minimal()
```
